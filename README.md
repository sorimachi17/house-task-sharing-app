...
同棲カップル2人のための家事記録・可視化アプリ。誰が・何を・いつやったかを記録し、草グラフ・時間割・集計で見える化する。家事にはポイントが設定されており、週単位でA/Bどちらが多く稼いだかを競える。
...
insert into chores (name, category, points, sort_order) values
  ('掃除機', '掃除', 2, 1),
  ('埃取り', '掃除', 1, 2),
  ('散らかり片づける', '掃除', 1, 3),
  ('トイレ掃除', '掃除', 2, 4),
  ...
  ('コンロ', 'キッチン', 2, 4),
  ('ゴミ出し(燃えるゴミ)', 'ゴミ出し', 1, 1),
  ('ゴミ出し(ペットボトル)', 'ゴミ出し', 1, 2),
  ('ゴミ出し(段ボール)', 'ゴミ出し', 1, 3),
  ('ゴミ出し(缶)', 'ゴミ出し', 1, 4),
  ('シャンプー・ソープ補充', '買い物・補充', 1, 1),
  ('R1買う', '買い物・補充', 1, 2),
  ('水買う', '買い物・補充', 1, 3),
  ('水やり', 'その他', 1, 1);
```

ポイントは手間の目安。アプリ内の設定画面(⚙️ → 家事一覧の✎)からいつでも編集できる。
...
update chores set points = 2 where name in
  ('掃除機','トイレ掃除','洗濯干す','洗濯畳む','お風呂掃除(湯舟)','お風呂掃除(洗い場)','お皿洗い(二人分)','コンロ');
update chores set points = 3 where name = 'シーツ洗い';
...
- **記録タブ**: 今週のポイント対決・今日の獲得ポイントを表示するスコアカード、その下にカテゴリ別の家事一覧をタップして記録
- **カレンダータブ**: 月表示。各日を上下2分割してA/Bそれぞれの実施回数を濃淡+件数で表示
- **週タブ**: 7:00〜24:00の時間割ビュー。いつ・誰が家事をしたかを時刻位置にチップ表示
- **集計タブ**: 今週/今月のA・B比較と、家事別の実施件数テーブル
- **設定(ヘッダーの⚙️)**: 表示名の変更、家事マスタの追加・編集・表示切り替え・ポイント調整
- **「＋ 記録」ボタン(全タブ共通)**: どの画面からでも家事選択シートを開いて記録できる

`both`(二人でやった)の記録は、集計・カレンダー・ポイント集計ともにA・B両方に1件(1ポイント分)ずつカウントされる。

家事ごとに目安時間(5分未満/5〜10分/10〜15分/15〜20分)とポイント(手間の目安、初期値は1〜3pt)を設定でき、同じ家事を選んだ記録は常に同じ値として扱われる(記録のたびに個別入力する必要はない)。
